...
| **Decide** | `/decide` researches a career fork with live evidence, grades source quality (recruiter data vs company disclosures), scores the options over a 20-year horizon, sets measurable kill criteria and keeps a decision log. `tools/signals.mjs` tracks lane counts month by month so those criteria can actually be checked. |
